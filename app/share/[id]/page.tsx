...
export default async function SharePage({ params }: SharePageProps) {
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-10">
      ...
      {/* Product info */}
      {product ? (
        <div className="mb-6 flex items-center gap-4 rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-900">
      ...
      <div className="mt-6 rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-900">
        ...
        <code className="block break-all text-xs text-slate-800 dark:text-slate-200">
          {`${process.env.NEXT_PUBLIC_BASE_URL ?? "http://localhost:3000"}/share/${id}`}
        </code>
      </div>
    </main>
  );
